...
export async function PATCH(req: NextRequest, { params }: RouteContext): Promise<NextResponse> {
  ...
  if (parsed.data.latitude != null && parsed.data.longitude != null) {
    coordsUpdate = { latitude: parsed.data.latitude, longitude: parsed.data.longitude }
  } else if ('address' in parsed.data) {
    const geocoded = parsed.data.address ? await geocodeAddress(parsed.data.address) : null
    coordsUpdate = geocoded ?? { latitude: null, longitude: null }
  }

  // eslint-disable-next-line @typescript-eslint/no-unused-vars
  const { latitude: _lat, longitude: _lng, ...updateData } = parsed.data

  const { data, error } = await supabase
    .from('places')
    .update({ ...updateData, ...coordsUpdate })
    .eq('id', placeId)
    .select()
    .single()
